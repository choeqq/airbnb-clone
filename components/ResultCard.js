...
export default function ResultCard({
  ...
  const scrollToImage = (i) => {
    imagesRef.current.scrollLeft = imagesRef.current.offsetWidth * i;
  };
  ...
  return (
    <CardContainer>
      <div ref={imagesRef} className="carousel">
        {imgSrc.map((url, i) => (
          <ImageComponent key={i} url={url} location={location} />
        ))}
      </div>
      {imgSrc?.length > 1 && (
        <div className="scroller">
          {imgSrc.map((img, i) => (
            <span
              key={i}
              className={currSlide === i ? "active" : null}
              onClick={(e) => {
                e.stopPropagation();
                scrollToImage(i);
              }}
      ...
      <div className="details">
        <div className="rating">
          <Star className="star" />
          {star} <small>({String(lat).split(".")[1].substring(0, 3)})</small>
        </div>
...
const ImageComponent = ({ i, url, location }) => {
