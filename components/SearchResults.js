...
import { resultImages } from "../data.js";
...
export default function SearchResults({
  ...
  return (
    <ResultsContainer className="hero">
      <div className="inner">
        <p className="details">
        ...
        <div className="results">
          {results.map((item, i) => (
            <ResultCard
              onClick={() =>
                setSelection({ lat: item.lat, long: item.long, index: i })
              }
              {...item}
              imgSrc={resultImages[i]}
              key={i}
            />
          ))}
        </div>
